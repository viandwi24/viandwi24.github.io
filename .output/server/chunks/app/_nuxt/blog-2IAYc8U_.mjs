...
import { a as useSeoMeta, b as useLoading, f as fetchDefaults, d as asyncDataDefaults, g as useRequestFetch, e as useNuxtApp, c as createError } from '../server.mjs';
import { defineComponent, withAsyncContext, mergeProps, unref, useSSRContext, computed, toValue, reactive, ref, shallowRef, toRef, getCurrentInstance, onServerPrefetch } from 'vue';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(((_a = unref(data)) == null ? void 0 : _a.data) || [], (type, i) => {
        ...
        ssrRenderList(type.feed.items || [], (item, j) => {
          ...
          _push(`<div class="flex flex-col gap-2"><div class="text-gray-100">${ssrInterpolate(item.title)}</div><div class="text-gray-500 text-xs">${ssrInterpolate(item.link)}</div><div class="text-gray-400 text-sm">${ssrInterpolate(`${item["content:encodedSnippet"]}`.substring(0, 200))}...</div></div></a>`);
        });
        _push(`<!--]--></div></div>`);
      });
